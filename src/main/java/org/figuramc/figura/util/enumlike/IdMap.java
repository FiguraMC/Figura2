package org.figuramc.figura.util.enumlike;
...
public class IdMap<K extends EnumLike, V> implements Map<K, V> {
    ...
    private class EntrySet extends AbstractIdMapBackingSet<IdEntry, Map.Entry<K, V>> {
        @Override K toKey(IdEntry item) { return item.key; }
        @Override IdEntry fromKey(K key) { return new IdEntry(key); }
    }
    private class IdEntry implements Map.Entry<K, V> {
        private final K key;
