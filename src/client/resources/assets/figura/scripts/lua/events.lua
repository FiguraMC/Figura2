...
--- @return Event
function Event.new()
    return setmetatable({queue = {}}, Event)
end
...
-- Create events by vararg:

--- Backing table tracking all events by name
--- @type table<string, Event>
local byName = {}
for _,name in ipairs({...}) do
    byName[name] = Event.new()
end

--- Special case functions in the "events" class new() will create an anonymous Event object.
--- @return Event
setmetatable(events, {
  -- events.something will return the Event object
  __index = byName,
  -- function events.something() will register a new function
  --- @param name string
  --- @param func function
  __newindex = function(_, name, func)
    if not byName[name] then byName[name] = Event.new() end
...
-- Return the table of events by name
return byName