package org.figuramc.figura.script_languages.molang.func;
...
public record ComparisonOperator(String name, BiConsumer<MethodVisitor, Label> tester) implements MolangFunction {
    ...
    public void checkArgs(List<MolangExpr> args, String source, int funcNameStart, int funcNameEnd) throws MolangCompileException {
        if (args.size() != 2)
            throw new MolangCompileException("figura.error.script.molang.compile.wrong_arg_count", source, funcNameStart, funcNameEnd, name(), "2", args.size());
        if (args.get(0).isVector() && args.get(1).isVector() && args.get(0).returnCount() != args.get(1).returnCount())
            throw new MolangCompileException("figura.error.script.molang.compile.vector_args_same_size", source, funcNameStart, funcNameEnd, name(), args.get(0).returnCount(), args.get(1).returnCount());
    }
