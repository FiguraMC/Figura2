...
public record FloatFunction(String name, int argCount, Consumer<MethodVisitor> floatFunc, boolean usesDouble) implements MolangFunction {
    ...
    public void checkArgs(List<MolangExpr> args, String source, int funcNameStart, int funcNameEnd) throws MolangCompileException {
        if (args.size() != argCount)
            throw new MolangCompileException("figura.error.script.molang.compile.wrong_arg_count", source, funcNameStart, funcNameEnd, name(), argCount, args.size());
        int vecSize = 1;
        for (var expr : args) {
            ...
            if (size != 1) {
                if (vecSize == 1) vecSize = size;
                    ...
                    throw new MolangCompileException("figura.error.script.molang.compile.vector_args_same_size", source, funcNameStart, funcNameEnd, name(), vecSize, size);
                }
            }
        }
    }
