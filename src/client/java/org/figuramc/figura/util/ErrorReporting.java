...
//    }
    ...
    public static void reportError(Throwable error) {
        if (error instanceof FiguraException figuraException) {
            // If it's a FiguraException, it already has a Component which we can show to chat
            Minecraft.getInstance().gui.getChat().addMessage(figuraException.component.copy().withStyle(ChatFormatting.RED));
            FiguraMod.LOGGER.error("An error occurred", figuraException);
        } else {
            // Internal error
            ...
            FiguraMod.LOGGER.error("INTERNAL FIGURA ERROR: PLEASE REPORT TO DEVS!", error);
        }
